...
function companySearch()
{
    $.ajax({
		...
		type:"POST",
		data:{
			"companysearch":$('input[type=text][name=companysearch]').val()
			},
		dataType:"json",
		...
		error: function(data)
		{
			$('input[type=text][name=companysearch]').val("Company search failed!");
		}
	});
}
...
$('tbody').on("click", ".material-icons", function(){
	...
	$.ajax({
		...
		error: function(data)
		{
			$('input[type=text][name=usersearch]').val("Company ban failed!");
		}		
	});
