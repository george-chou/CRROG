...
function userSearch()
{
    $.ajax({
		...
		type:"POST",
		data:{
			"usersearch":$('input[type=text][name=usersearch]').val()
			},
		dataType:"json",
		...
		error: function(data)
		{
			$('input[type=text][name=usersearch]').val("User search failed!");
		}
	});
}
...
$('tbody').on("click", ".material-icons", function(){
	...
	$.ajax({
		...
		error: function(data)
		{
			$('input[type=text][name=usersearch]').val("User ban failed!");
		}		
	});
